...
The parameter provided **FASTEST_START: "yes"** will download the latest full-hour blockchain snapshot, and the remaining blocks will be synchronized automatically. You can check server status and current block height via this API: [http://{YOUR_NODE_IP}:9000/api/v1/status/peer](http://{your_node_ip}:9000/api/v1/status/peer).
